...
> | `utils.tcl` のユーティリティ（`::sshcomm::utils`） | **現役** | 本体が `askpass-helper` で依存。[api-reference.md](api-reference.ja.md) §6 |
...
> [improvement-notes.md](todo/improvement-notes.ja.md) §3 を参照。
...
→ 改善候補。[improvement-notes.md](todo/improvement-notes.ja.md) を参照。
