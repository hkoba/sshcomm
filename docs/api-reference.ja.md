...
| `-control-channel` | `pipe` | `socket` で制御チャネルを専用ソケットへハンドオフし、リモート stdout をアプリへ開放（[architecture.md](architecture.ja.md) §3）|
...
- 接続不要のユニットテスト（`cget`/`cookie`/`unix sshcmd` の文字列生成）と、
  実 SSH を要する統合テストが混在している（[improvement-notes.md](todo/improvement-notes.ja.md) 参照）。
- **テスト方針**: 【実験的】機能（`gcloud sshcmd`・`-sshcmd-platform-options`・plugin 機構）は
  **テスト作成を免除**。一方 `windows sshcmd` は現役・重要のためテスト追加が望ましい
  （[README.md](README.ja.md) のステータス凡例、improvement-notes §5 参照）。
