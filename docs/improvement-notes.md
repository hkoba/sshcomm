...
> （`sshcomm.tcl:363`）や §8 の `remote eval`/`lread` 混線を **根本から解消する基盤改善** でもある。
...
制御チャネル `mySSH` は `open [list | ssh ... tclsh] w+`（`sshcomm.tcl:293`）で開いた
**SSH プロセスの stdin/stdout パイプ＝リモート `tclsh` の stdin/stdout そのもの** である。
この1本の上に、制御プロトコルが多重に乗っている。

- リモート側: `fileevent stdin readable [list ... control stdin]`（`:759`）で **stdin を制御コマンド受口** に占有。
- リモート側: `keepalive`（`:857`）が **stdout** へ `pid/時刻` を定期出力。
- ローカル側: `remote eval` の同期応答を **stdout** から `remote lread`（`:343`）で読む。
...
`forward new raw`（`:412`）＋ `accept__raw`（`:807`）は、Cookie 認証された **双方向の生 TCP ソケット**
...
- **`remote::control` は既に汎用**: `control fh args`（`:862`）は任意チャネル `fh` から完全な
...
- **`comm init`（`:443`）の「既存ソケットにプロトコルを手で被せる」技法**が、制御ソケットへ
  `fileevent` ベースの制御プロトコルを組むときの参考になる。

### 0.6 ローカル側の変更点

- `mySSH` の `readable` ハンドラ（`remote readable`、`:479`）を、制御プロトコル解釈から
  **アプリ出力のパススルー／コールバック** へ変更（例: リモート stdout を読むチャネル/コールバックAPIを提供）。
- **stderr の開放**: 現状は `-ssh-verbose` 時のみ `2>@ stderr`（`:289`）。アプリに stderr を渡すなら、
  常時 Tcl チャネルへ向ける（`open` 時のリダイレクト設計を見直す）必要がある。
- **死活監視**: 現在は SSH パイプの `eof`（`:483`）で検出。keepalive を制御ソケットへ移すと、
  SSH パイプとは別に **制御ソケットの生存監視** が要る。
- **終了シーケンス**: destructor は `puts $mySSH "exit"`（`:217`）で SSH パイプ経由にリモートを落とす。
  制御を別ソケットに移したら、exit をどのチャネルから送るか整理する。

### 0.7 副次効果（既存課題の同時解消）

- §1 `sshcomm.tcl:363` の `remote redefine`×keepalive 競合 → 解消。
- §8 の `remote eval`/`lread` の stdout 混線 → 解消。
- §1 `:776`/`:777`（accept の非ブロッキング read・長さ制限）も、制御チャネル実装を作り直す際に同時対処しやすい。
...
- セキュリティ面はむしろ向上: 制御チャネルは `uplevel #0` で任意コードを実行するので、Cookie 認証＋
  localhost 限定（`:770`）が効く専用ソケットに乗せるのは妥当（§2 とも整合）。
...
2. **地ならし(a) テスト整備**: 接続不要テストの分離＋CI 化、`windows`/`gcloud` sshcmd の
   文字列生成テスト追加（§5）。リグレッション検知の土台。
3. **地ならし(b) 明示済みの小修正**: Cookie 長さ制限・非ブロッキング read（`:776`-`777`）など。
